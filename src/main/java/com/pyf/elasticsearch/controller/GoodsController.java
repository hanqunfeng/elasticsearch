...
/**
 * ${DESCRIPTION}
 * Created by hanqf on 2018/11/8 14:28.
 */
...
public class GoodsController {
    ...
    public String save(){
        GoodsInfo goodsInfo = new GoodsInfo();
        goodsInfo.setTitle("<<咏鹅>>");
        goodsInfo.setContent("鹅鹅鹅，曲项向天歌，白毛浮绿水，红掌拨清波");
        goodsInfo.setWeight(100);
        goodsInfo.setUserId(500);
        goodsRepository.save(goodsInfo);
        return "success";
    }
    ...
    @PostMapping("/update")
    public String update(String id,GoodsInfo goodsInfo ){
        goodsInfo.setId(id);
        goodsRepository.save(goodsInfo);
        return "success";
    }
    ...
    @GetMapping("/search")
    public List<GoodsInfo> getSearch(String title,String content,String kind) {
        Map<String,Object> map = new HashMap<>();
        map.put("title",title);
        map.put("content",content);
        SearchQuery searchQuery = ElasticsearchUtil.getSearchQuery(map,kind);
        Iterator<GoodsInfo> searchResult = goodsRepository.search(searchQuery).iterator();
        List<GoodsInfo> list = IteratorUtils.toList(searchResult);
        return list;
    }
    ...
    //http://localhost:8888/getGoodsList?query=如梦令
    //http://localhost:8888/getGoodsList?query=商品&pageNumber=1
    //根据关键字"query"去查询列表，title或者content包含的都查询
    ...
    private SearchQuery getEntitySearchQuery(int pageNumber, int pageSize, String searchContent) {
        ...
        SearchQuery searchQuery = ElasticsearchUtil.getSearchQuery(pageable,searchContent,"title","content");
        return searchQuery;
    }
